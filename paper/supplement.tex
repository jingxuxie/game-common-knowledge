...
\section{Correlated Public Certificates}

For each publicly observed context $r$, let $\Omega_r$ be a finite or measurable outcome space with arbitrary distribution $\nu_r$. Outcome $\omega$ determines a set $C_r(\omega)\subseteq[m]$ of features that emit a decisive certificate. If a selected certificate is present, all agents can identify and implement a context-optimal compatible action, normalized to success one. If no selected certificate is present, the best decentralized value is $q_r$. Correlation among feature outcomes is entirely encoded in the random set $C_r(\omega)$.

\begin{theorem}[Correlated certificate submodularity]\label{thm:cert-supp}
The coordination gain
\[
F(S)=\sum_r w_r(1-q_r)
\int_{\Omega_r}\ind\{S\cap C_r(\omega)\ne\varnothing\}\,d\nu_r(\omega)
\]
is normalized, monotone, and submodular for arbitrary joint certificate distributions.
\end{theorem}
\begin{proof}
Fix $r$ and $\omega$, and abbreviate $C=C_r(\omega)$. Define
\[
h_C(S)=\ind\{S\cap C\ne\varnothing\}.
\]
This function is normalized and monotone. For $A\subseteq B$ and $j\notin B$, its marginal is
\[
h_C(A\cup\{j\})-h_C(A)
=\ind\{j\in C,\ A\cap C=\varnothing\}.
\]
The analogous marginal at $B$ replaces the condition $A\cap C=\varnothing$ by the stronger condition $B\cap C=\varnothing$. Hence the marginal at $A$ is at least the marginal at $B$, proving submodularity for every realized certificate set. Nonnegative weighted sums and expectations preserve normalization, monotonicity, and submodularity. No factorization or conditional-independence assumption is used.
\end{proof}

\begin{corollary}[Cardinality and cost budgets]\label{cor:budgets}
Under $|S|\le k$, marginal-value greedy achieves at least $(1-1/e)F(S^\star)$. Under a positive-cost budget $\sum_{j\in S}c_j\le B$, a polynomial-time $(1-1/e)$ approximation exists.
\end{corollary}
\begin{proof}
The cardinality claim is the classical theorem of Nemhauser, Wolsey, and Fisher \cite{NemhauserEtAl1978}. The cost-budget claim follows from Sviridenko's monotone-submodular knapsack algorithm \cite{Sviridenko2004}.
\end{proof}

\begin{corollary}[Independent revelation]\label{cor:independent}
If feature $j$ is in $C_r(\omega)$ independently with probability $p_{rj}$, then
\[
F(S)=\sum_r w_r(1-q_r)\left[1-\prod_{j\in S}(1-p_{rj})\right].
\]
\end{corollary}
\begin{proof}
The selected set fails to contain a certificate in context $r$ exactly when every selected feature fails, which has probability $\prod_{j\in S}(1-p_{rj})$.
\end{proof}

\begin{proposition}[Marginals do not identify redundancy]\label{prop:marginals-supp}
For every $p\in(0,1)$, there are two two-feature certificate systems in which each singleton has gain $p$, but the pair has gain $p$ in one system and $1-(1-p)^2$ in the other.
\end{proposition}
\begin{proof}
Use one context, baseline zero, and unit value after a certificate. In the correlated system, let $C(\omega)=\{1,2\}$ with probability $p$ and $C(\omega)=\varnothing$ otherwise. Each singleton and the pair then hit the certificate set with probability $p$. In the independent system, include each feature in $C(\omega)$ independently with probability $p$. Each singleton again has gain $p$, while the pair succeeds unless both events fail, giving $1-(1-p)^2$.
\end{proof}

\begin{theorem}[Value-greedy can be arbitrarily bad]\label{thm:greedy-supp}
For every $\rho>0$, there is a three-feature, budget-two BPOD instance for which marginal-value greedy obtains less than $\rho$ times the optimal coordination gain.
\end{theorem}
\begin{proof}
...
All experiments run on CPU using NumPy, SciPy's HiGHS MILP interface, and Matplotlib. Fixed random seeds appear in the scripts. Unit tests compare the joint MILP against exhaustive feature-set optimization on small instances, verify all analytic values, and exhaustively check diminishing returns on small correlated-certificate instances.
...
We draw 250 instances with eight contexts, twelve sensors arranged into three common failure domains, and budget four. In each instance one randomly chosen domain has uptime in $[0.52,0.68]$ and highly reliable local detectors in $[0.91,0.995]$; the other domains have uptime in $[0.88,0.98]$ and local detector reliabilities in $[0.48,0.68]$. For a selected set $S$, the exact probability of no certificate in context $r$ is
\[
\prod_d\left[(1-a_{rd})+a_{rd}\prod_{j\in S\cap G_d}(1-p_{rj})\right],
\]
where $a_{rd}$ is domain uptime and $G_d$ its sensors. This creates positive within-domain correlation. The misspecified baseline keeps each marginal probability $a_{r,d(j)}p_{rj}$ but multiplies sensor failures independently. Exhaustive subset search supplies the true optimum. The experiment reports true-objective ratios and the number of represented failure domains.
...
\midrule
Independent-revelation greedy mean & 0.997 & 250 \\
Independent-revelation greedy minimum & 0.945 & 250 \\
Correlated aware-greedy mean & 1.000 & 250 \\
Correlated independence-model mean & 0.864 & 250 \\
Correlated independence-model minimum & 0.616 & 250 \\
...
The access-structure theorem is an expressivity statement, not a claim that engineered sensor suites are adversarial secret-sharing schemes. Its purpose is to rule out universal low-order complementarity assumptions. The correlated-certificate theorem gives one transparent sufficient substitutes condition, but it is not necessary. Its guarantee survives arbitrary dependence among certificate events because each realized outcome is a coverage function; it does not cover settings in which several partial messages must be combined before any one is decision-sufficient.
